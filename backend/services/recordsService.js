const recordsQuery = require('../queries/recordsQuery');
...
const tratamientoQuery = require('../queries/tratamientoQuerys');
const examenFisicoQuery = require('../queries/examen_fisicoQuerys');
const antecedenteQuery = require('../queries/antecedenteQuerys');
...
async function guardar_historia_clinica(historia) {
    try {
        ...
        await recordsQuery.guardar_historia_clinica(nuevaHistoria);

        // actualizar el estado de la cita medica
        await appointmentsQuery.cambiarEstado(historia.cita_medica.id_cita_medica);
...
async function actualizarHistoria(historia) {
    try {
        ...
        await recordsQuery.actualizarHistoria(nuevaHistoria);
...
async function listarHistoriaClinicaPorMedico(id_medico) {
    try {
     const citas = await appointmentsQuery.listarHistoriaClinicaPorMedico(id_medico);
     return citas;
    } catch (error) {
...
async function listarPacientesCorrespondientesAMedico(id_usuario) {
    try {
     const citas = await appointmentsQuery.listarPacientesCorrespondientesAMedico(id_usuario);
     return citas;
    } catch (error) {
        console.error('Error al listar las citas.', error);
    }
...
async function listarHistoriaClinicaPorPaciente(id_usuario) {
    try {
     const citas = await appointmentsQuery.listarHistoriaClinicaPorPaciente(id_usuario);
     return citas;
    } catch (error) {
        console.error('Error al listar las citas.', error);
    }
...
async function listarTodoPacientes() {
    try {
     const citas = await appointmentsQuery.listarTodoPacientes();
     return citas;
    } catch (error) {
